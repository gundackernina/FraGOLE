 ...
 */
function connectWaypoints(wps, jsonItemContentConnectWaypoints, gameController) {
    if (usedDefaultWaypoints) {
        gameItems.connectWaypoints(gameController);
        ...
        connectPaths(wps, gameController);
        connectSingleWaypoints(jsonItemContentConnectWaypoints, wps, gameController);
    }
}
 ...
 * @param {Waypoint} wps 
 */
function connectPaths(wps, gameController) {
    ...
    for (let path in paths) {
        Lib.connectWaypoints(paths[path], gameController, true);
    }
}
 ...
 * @param {Waypoints} wps 
 */
function connectSingleWaypoints(items, wps, gameController) {
    for (let i = 0; i < items.length; i++) {
        let connect = items[i];
        let from = connect.wpFromId;
        let to = connect.wpToId;
        console.log('WaypointFrom:' + from + ' WaypointTo:' + to);
        Lib.connectWaypoints([wps[from], wps[to]], gameController, true);
    }
}
