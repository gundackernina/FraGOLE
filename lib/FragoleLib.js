...
*/
function connectWaypoints(wpList, gameController, bothWays=false) {
    ...
    for (let wp of wpList) {
        ...
        let name = currWp.id + "-" + wp.id;
        let cmd = ['drawLine', name, currWp.x, currWp.y, wp.x, wp.y];
        gameController.rpcListOrAll(gameController.joinedPlayers, cmd)
...
/**
 * Gets the Server Port from the process.argv start Arguments
 * @param {process.argv} args 
 */
function getServerPort(args) {
    let defaultPort = 80;
    let port;
    if (args.length > 2) {
    ...
    //If there is no Port in args, then return defaultPort (80)
    return defaultPort;
}
module.exports.getServerPort = getServerPort;

/**
 * Gets the Configuration Directory from the process.argv start Arguments
 * The directory is needed if you want to configure your game with json
 * files
 * @param {process.argv} args 
 */
function getConfigurationPath(args) {
    let path;   
    if (args.length > 3) {
        path = args[3];
        try {
            ...
            console.log('No valid path name, using defaultConfiguration instead');
        }  
    }
}
module.exports.getConfigurationPath = getConfigurationPath;
